...
export class SubagentService {
    ...
    subUser(company, iata, no_branch, c_person, c_designation, c_email, c_phone, o_address, o_city, country, o_pin, contact_no, email, pswd, profile_picture,
        licence_number, license_picture, currency_code): Observable<any> {
        //console.log('hi');
        const url = myGlobals.ExternalUrl + 'dashboard/subagent_create';
    ...
    getSubAgentList(): Observable<any> {
        const url = myGlobals.ExternalUrl + 'dashboard/sub_agent';
        const token = localStorage.getItem('token');
        const params = new HttpParams().set('token', token);
    ...
    getSubAgentActive(id): Observable<any> {
        const url = myGlobals.ExternalUrl + 'dashboard/active_subagent/' + id;
        const token = localStorage.getItem('token');
        const params = new HttpParams().set('token', token);
    ...
    getSubAgentInactive(id): Observable<any> {
        const url = myGlobals.ExternalUrl + 'dashboard/inactive_subagent/' + id;
        const token = localStorage.getItem('token');
        const params = new HttpParams().set('token', token);
